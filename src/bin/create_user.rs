...
fn main ()
{
    ...
    options.reqopt("e", "email", "e-mail address for the new user", "alyssa@example.com");
    options.reqopt("p", "password", "password for the new user", "MySecretPassword");
    options.reqopt("u", "username", "username for the new user", "alyssatest");
    options.optopt("s", "summary", "a summary (bio) for the new user", "I am a Kibou Test-Actor, how are you doing?");
