use actor;
use database;
use kibou_api::account as account;
...
fn follow()
{
    ...
    let activitypub_followers: serde_json::Value = serialized_test_actor.followers["activitypub"].clone();
    let test_actor_follow_data: Vec<serde_json::Value> = serde_json::from_value(activitypub_followers).unwrap_or_else(|_| vec![]);
